...
class TextCNN(object):
    ...
    def __init__(
        ...
        with tf.device('/cpu:0'), tf.name_scope('embedding'):
            # define W [vocab_size x embedding_size], W can put the vocabulary map to embedding (high dimension -> low dimension)
            self.W = tf.Variable(
                initial_value=tf.random_uniform(shape=[vocab_size, embedding_size], minval=-1.0, maxval=1.0),
                name='W')
